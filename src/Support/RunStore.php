...
class RunStore
{
    ...
    private function launch(string $artisan, string $argString, string $log): void
    {
        ...
        // ── Robust detachment via a self-detaching CLI runner ────────────────
        // The naive `nohup … &` child stays in php-fpm's process group / session,
        // so when the web worker finishes the request the OS can signal-kill it —
        // the run then dies right after start with no output, no error, and no
        // sentinel (exactly the "stuck" symptom). macOS has no `setsid`, but the
        // CLI php WE spawn can detach itself: it forks, calls posix_setsid() to
        // start a brand-new session, then runs artisan. That grandchild is no
        // longer tied to fpm and survives independently.
        $runner = $this->writeRunnerScript($log, $base, base_path());

        if ($runner !== null) {
            $shell = sprintf('%s %s >/dev/null 2>&1 &', escapeshellarg($php), escapeshellarg($runner));
        } else {
            // Fallback: original nohup approach (still works under the CLI SAPI).
            $inner = sprintf(
                '{ %s ; echo "%s$?" ; } >> %s 2>&1',
                $base,
                self::SENTINEL,
    ...
    /**
     * Write a tiny self-detaching runner script next to the log and return its
     * path (or null if it could not be written, so the caller can fall back).
     *
     * The runner double-forks + posix_setsid() so it leaves php-fpm's process
     * group/session entirely, then runs the real command with its output +
     * completion sentinel streamed to the log. If pcntl/posix are unavailable it
     * still runs the command inline (backgrounded by the caller's `&`).
     */
    private function writeRunnerScript(string $log, string $baseCommand, string $cwd): ?string
    {
        $runnerPath = dirname($log) . '/runner.php';
        ...
        $code = "<?php\n"
            . "// Auto-generated by CodeGuardian RunStore. Detaches from php-fpm, then runs.\n"
            . '$cmd = ' . var_export($inner, true) . ";\n"
            . '$cwd = ' . var_export($cwd, true) . ";\n"
            . '$log = ' . var_export($log, true) . ";\n"
            . "if (function_exists('pcntl_fork')) {\n"
            . "    \$pid = @pcntl_fork();\n"
            . "    if (\$pid > 0) { exit(0); }            // fpm's direct child exits now\n"
            . "    if (function_exists('posix_setsid')) { @posix_setsid(); }  // new session\n"
            . "    if (function_exists('pcntl_fork')) { \$p2 = @pcntl_fork(); if (\$p2 > 0) { exit(0); } }\n"
            . "}\n"
            . "\$p = proc_open(\$cmd, [0=>['file','/dev/null','r'],1=>['file','/dev/null','w'],2=>['file','/dev/null','w']], \$pipes, \$cwd);\n"
            . "if (is_resource(\$p)) { proc_close(\$p); } else { @file_put_contents(\$log, \"\\nFailed to spawn command.\\n" . self::SENTINEL . "1\\n\", FILE_APPEND); }\n";

        return @file_put_contents($runnerPath, $code) !== false ? $runnerPath : null;
    }
